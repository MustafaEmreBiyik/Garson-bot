...
**Son güncelleme:** 24 Temmuz 2026 | **Sürüm:** 5.23
...
**24 Temmuz 2026 durumu (patron incelemesine hazır):** wbot_v5 (LLM) GGUF'a
çevrildi, Jetson'a deploy edildi, `eval_gguf.py` ile test edildi — 28/32
(%87) ana, 32/39 (%82) v4-targets; **E09 regresyon şüphesi GEÇERSİZ
çıktı** (gerçekte GEÇİYOR). wbot_tr_v2 (TTS) 970 cümlelik yeni korpusla
sıfırdan fine-tune edildi (epoch 5679→6599, ~15.5 saat, kesintisiz),
ONNX export edildi. Kullanıcı kör A/B testini (`tts_ab_out/wbot_v2_ab/index.html`)
dinledi ve v2'ye onay verdi. **İkisi de production'a BAĞLANDI** (24 Temmuz
2026): `llama_cpp_backend.py::_GGUF_FILENAME` → wbot_v5, `tts.py::_PIPER_MODEL_CANDIDATES`
→ wbot_tr_v2.onnx öncelikli (v1 hâlâ yedek olarak listede, dosya silinmedi).
Değişiklik commit'lendi + Jetson'a pull edildi. Detay: görev tablosu #33-35 + "3) wbot_tr overfit
hipotezi" bölümü.
...
### 3) wbot_tr overfit hipotezi → **tam retrain'e karar verildi + tamamlandı** (18-24 Temmuz 2026) ✅

Checkpoint-A/B planı (yukarıdaki eski not) terk edildi — bunun yerine
**sıfırdan, çok daha büyük bir korpusla tam retrain** yapıldı: 970 cümle
(694 v2 + 276 ek — 6 yeni ton kategorisi: şaşkınlık, ısrarlı-sabırlı
tekrar, kısa yanıtlar, mahrem ton, hızlı diyalog, karışık-duygu uzun
cümle), yeni mikrofon (cihaz eleme testiyle doğrulandı — bkz. dosya
sonu `test_all_mics.py`/`test_mic_level.py`), `scripts/tts_corpus_v2_ek.txt`.

**Eğitim notebook'unda (`wbot_piper_training_v2.ipynb`) bulunan/düzeltilen
gerçek bug'lar:**
- Preprocess/Eğitim/Export hücrelerinde `DRIVE_ROOT` hâlâ v1'in klasörünü
  (`wbot-tts`) gösteriyordu → eğitim dfki-medium'dan değil **v1'in eski
  (epoch 6799, overfit şüpheli) checkpoint'inden devam edecekti. `wbot-tts-v2`
  olarak düzeltildi.
- Export hücresi çıktıyı `wbot_tr.onnx` (v2 eksiz) yazıyordu, Test/İndir
  hücreleri `wbot_tr_v2.onnx` arıyordu — eşleşmeyecekti. Birleştirildi.
- **`piper-phonemize`'ın Linux için Python 3.12 wheel'i yok** (PyPI'de
  yalnız macOS cp312) — Colab artık 3.12 kullanıyor. Çözüm: izole Python
  3.10 venv (`deadsnakes` PPA), tüm `piper_train` çağrıları `VENV_PY`
  üzerinden. `setuptools` eksikliği (`pkg_resources` ModuleNotFoundError,
  yeni venv'ler artık otomatik getirmiyor) ayrıca düzeltildi.
- Taban checkpoint boyut kontrolü `>100MB` idi — 149MB'lık yarıda kesilmiş
  bir indirmeyi "tamam" sayıp geçiyordu (gerçek dosya 846MB). `>800MB`'a
  sıkılaştırıldı.
- Hücre 1'in "Factory reset runtime çözer" tavsiyesi yanlıştı (kernel
  Python sürümünü değiştirmez) — düzeltildi.

**Eğitim sonucu:** dfki-medium (epoch 5679) checkpoint'inden
`--resume_from_checkpoint` ile fine-tune başlatıldı (piper_train kaynağı
doğrulandı: tekli-konuşmacı için bu DOĞRU/tasarlanmış kullanım —
`--resume_from_single_speaker_checkpoint` farklı bir senaryo, tekli→çoklu
konuşmacı dönüşümü için). Epoch sayacı checkpoint'ten miras alındığından
5679'dan devam etti; **epoch 6599'a kadar (~900 yeni epoch) T4'te, FP32
hassasiyette, ~15.5 saat kesintisiz** çalıştı — her 100 epoch tam olarak
~1sa42dk arayla, hiç kesinti/çökme olmadan (10 checkpoint, Drive'a
otomatik senkron). ONNX export edildi: `wbot_tr_v2.onnx` (60.6 MB) +
`.onnx.json`, hem dev makineye hem Jetson'a (`robot_waiter_ai/models/`)
kopyalandı — **`wbot_tr.onnx` (v1) production'da değiştirilmedi**, v2
ayrı dosya olarak duruyor.

**Kör A/B testi hazır, kullanıcı dinlemesi bekleniyor:** `tts_ab_out/wbot_v2_ab/index.html`
— aynı 12 görülmemiş cümle, v1/v2 karışık ("Aday 1/2", cevap anahtarı
altta gizli), dfki referans + insan sesi hatırlatıcı dahil. Karar kulakla;
kod/prompt tarafında ekstra iş gerekmiyor.
...
| 33 | wbot_tr overfit hipotezi testi → tam retrain kararı (checkpoint A/B yerine) | 🟡 Orta | ✅ Tamamlandı — 24 Temmuz 2026, detay yukarıda "3) wbot_tr overfit hipotezi → tam retrain'e karar verildi" bölümünde. 970 cümle (694+276 ek), yeni mikrofon, notebook'ta 5 gerçek bug bulunup düzeltildi (DRIVE_ROOT yanlış klasör, ONNX ad uyuşmazlığı, Python 3.12/piper-phonemize uyumsuzluğu → izole venv, setuptools eksikliği, boyut kontrolü zayıflığı). Eğitim epoch 5679→6599 (~900 yeni epoch, ~15.5 saat, T4/FP32, kesintisiz), `wbot_tr_v2.onnx` export edildi, Jetson+dev'e kopyalandı (production DEĞİŞMEDİ). Kör A/B sayfası hazır: `tts_ab_out/wbot_v2_ab/index.html`, kullanıcı dinlemesi bekleniyor |
| 34 | wbot_v5 GGUF dönüşümü + Jetson deploy + eval | 🔴 Kritik | ✅ Tamamlandı — 22-24 Temmuz 2026. `Qwen3-4B-wbot_v5-Q4_K_M.gguf` (2.5GB) Colab'da export edildi, Jetson'a (`/home/emk/models/`) ve dev'e kopyalandı. `eval_gguf.py` Jetson'da (geçici olarak `llama_cpp_backend.py`'nin `_GGUF_FILENAME`'i v5'e çevrilip test sonrası v4'e geri alındı — o an production hâlâ v4'tü): ana 32 senaryo **28/32 (%87)**, `--v4-targets` 39 senaryo **32/39 (%82)**. **E09 (Hamburger var mı?) → GEÇTİ** — daha önce ham adapter smoke test'te görülen olası regresyon GGUF/Jetson'da DOĞRULANMADI, gerçek değilmiş. Kalan: E01, E19, E24, E16 (E19/E24 zaten kod guard'ıyla üretimde kapatılıyor) + yeni V05/V06/V07 (HATA_TRIYAJ.md'ye tam triyaj formuyla eklendi — üçü de "kabul edilen fallback/izlenen", acil değil) |
| 35 | Production'ı wbot_v5 (LLM) + wbot_tr_v2 (TTS)'e bağlama | 🔴 Kritik | ✅ Tamamlandı — 24 Temmuz 2026. Kullanıcı TTS kör A/B testini dinleyip v2'yi onayladı. `llama_cpp_backend.py::_GGUF_FILENAME` → `Qwen3-4B-wbot_v5-Q4_K_M.gguf`; `tts.py::_PIPER_MODEL_CANDIDATES` → `wbot_tr_v2.onnx` listenin başına eklendi (v1 yedek olarak listede kaldı, dosya silinmedi — kolay geri dönüş). Commit + push + Jetson pull tamamlandı |
...
> ⚠️ Bu bölüm wbot_v3 döneminden kalma, güncellenmedi bırakılmıştı — asıl
> güncel yol haritası artık **YOL_HARITASI_SONRAKI_ADIMLAR.md** dosyasında
> (24 Temmuz 2026, patron incelemesi öncesi hazırlandı). Aşağıdaki zincir
> yalnızca tarihsel referans:

```
[ESKİ DURUM — 13 Tem] Jetson demo çalışıyor (wbot_v3, 31/32 %96 eval), wbot_v4_train.jsonl hazır (3605 kayıt)
    ↓
...
**[GÜNCEL DURUM — 24 Tem]:** wbot_v4→v5 (LLM) ve wbot_tr→v2 (TTS) tamamlandı,
ikisi de dosya olarak Jetson'da hazır, production'a bağlanması TTS kör A/B
kararına + patron dönüşüne bağlı. Detay: `YOL_HARITASI_SONRAKI_ADIMLAR.md`.
