...
**24 Temmuz 2026 durumu (patron incelemesi öncesi):** wbot_v5 (LLM) GGUF'a
...
ONNX export edildi. **İkisi de dosya olarak Jetson'da ve dev makinede
hazır AMA production'a henüz bağlanmadı** — `demo_usb.py`/`tts.py`/
`llama_cpp_backend.py` hâlâ eski (v4 LLM + v1 TTS) modelleri kullanıyor.
Karar bekleyen tek şey: TTS kör A/B testinin (`tts_ab_out/wbot_v2_ab/index.html`)
kullanıcı tarafından dinlenip v2'nin v1'den gerçekten iyi olup olmadığına
karar verilmesi — bu karardan sonra iki modeli birlikte production'a
bağlama işi yapılacak. Detay: görev tablosu #33-34 + "3) wbot_tr overfit
hipotezi" bölümü.
