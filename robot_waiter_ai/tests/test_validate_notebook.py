...
def test_missing_heading_error_includes_notebook_name(tmp_path):
    notebook = {
        "nbformat": 4,
        "cells": [
            {
    ...
    temp_path = tmp_path / "tmp_missing_heading_test.ipynb"
...
def test_hey_garson_wakeword_notebook_validates_successfully():
    notebook_path = (
        Path(__file__).resolve().parents[1]
        / "notebooks"
    ...
    assert notebook_path.exists()
    assert ok, "\n".join(errors)
    assert len(get_required_headings(notebook_path)) == 11
